...
Func InitHook()
	...
	if not $HookHandleI Then MsgBox(0, "Error", "Failed to hook KeyboardLLProc")
	$HookHandleJ = SetDllGlobalWindowsHookEx($WH_MOUSE_LL,$hGUI,"MouseLLProc")
	if not $HookHandleJ Then MsgBox(0, "Error", "Failed to hook MouseLLProc")
EndFunc
Func LockSystem()
	HookKey(1)
	DisableTaskMgr(1)
